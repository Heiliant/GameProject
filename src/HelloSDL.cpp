...
void Init(SDL_Window* &window, SDL_Renderer* &renderer) {
	...
	SDL_SetRenderDrawColor(renderer, 255, 0xFF, 0xFF, 0xFF);
}
...
int main(int argc, char *argv[]) {
	...
	try {
		...
		while (!quit) {
			if (!SDL_PollEvent(&e)) if (e.type == SDL_QUIT) quit = true; //primero inputs
			SDL_RenderClear(renderer);									//limpiamos la pantalla
																		//pintamos lo que toca en el renderer
			...
			//y este se renderiza en la pantalla
